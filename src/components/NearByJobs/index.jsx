...
const NearByJobs = ({ jobType, pref, muni, path, jobpost_id }) => {
  const { user } = useAuth();
      ...
      if (response.data?.jobposts) {
        setJobs(
          response.data.jobposts
            .filter((job) => job.jobpost_id !== jobpost_id)
            .slice(0, 10)
        );
      } else {
        setJobs([]);
  ...
  return (
    <div className="container mx-auto py-4 px-4 md:px-8 bg-white rounded-lg shadow-lg">
