...
const JobPostEdit = () => {
    ...
    const getJobPost = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/jobpost/${jobPostId}`);
            setJobPost(response.data.jobpost);
            setJobPostType(
                Object.keys(JobType.医科).includes(response.data.jobpost.type) ? "医科" :
                Object.keys(JobType.歯科).includes(response.data.jobpost.type) ? "歯科" :
                Object.keys(JobType.薬剤師).includes(response.data.jobpost.type) ? "薬剤師" :
                Object.keys(JobType.看護婦).includes(response.data.jobpost.type) ? "看護婦" :
                Object.keys(JobType.診療放射線技師).includes(response.data.jobpost.type) ? "診療放射線技師" :
                Object.keys(JobType.診療放射線技師).includes(response.data.jobpost.type) ? "診療放射線技師" :
                ""
            );
            ...
            setJobPostQualificationContent(response.data.jobpost.qualification_content);
            setJobPostQualificationWelcome(response.data.jobpost.qualification_welcome);
...
console.log(jobPost);
    ...
    const handleSave = async () => {
        ...
        const response = await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/jobpost/${jobPostId}`, JobPostData);
    ...
    const handleRequestAllow = async () => {
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/jobpost/pending/${jobPostId}`);
        if (response.data.error) message.error(response.data.error);
        ...
        <>
            <div className="w-full min-h-screen flex flex-col p-4 bg-white rounded-lg mb-8">
                ...
                <div className="flex items-start mt-4">
                    ...
                    <div className="flex items-center justify-start gap-2">
                        {jobPostPicture.length == 0 && <img src={jobPostPictureUrl} alt="施設写真" className="w-32 h-32 rounded-lg"/>}
                        <Upload
                            name="avatar"
                ...
                <div className="flex items-start mt-4">
                    ...
                    <EditorComponent editorValue={jobPostSubDescription} onEditorChange={(value) => setJobPostSubDescription(value)} editorStyle={editorStyle}/>
                </div>
                <div className="flex items-start mt-4 desireEmployment">
                    ...
                    <Checkbox.Group options={workItemOptions} value={jobPostWorkItem} onChange={(value) => setJobPostWorkItem(value)} className="w-4/5" />
                </div>
            ...
            >
                <div className="flex flex-col">
                    ...
                    <Link to="/customers/facility" className="text-center text-blue-500 mt-4">求人一覧へ戻る</Link>
                </div>
            </Modal>
        </>
    )
