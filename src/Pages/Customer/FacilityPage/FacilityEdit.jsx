...
const FacilityEdit = () => {
    ...
    const getFacility = useCallback(async () => {
        try {
            ...
            setFacilityJobType(
                Object.keys(JobType.医科).includes(response.data.facility.job_type[0]) ? "医科" :
                Object.keys(JobType.歯科).includes(response.data.facility.job_type[0]) ? "歯科" :
                Object.keys(JobType.薬剤師).includes(response.data.facility.job_type[0]) ? "薬剤師" :
                Object.keys(JobType.看護婦).includes(response.data.facility.job_type[0]) ? "看護婦" :
                Object.keys(JobType.診療放射線技師).includes(response.data.facility.job_type[0]) ? "診療放射線技師" :
                Object.keys(JobType.診療放射線技師).includes(response.data.facility.job_type[0]) ? "診療放射線技師" :
                ""
            );
            ...
            setFacilityEstablishmentDateYear(response.data.facility.establishment_date.split("-")[0]);
            setFacilityEstablishmentDateMonth(response.data.facility.establishment_date.split("-")[1]);
    ...
    const handleSave = async () => {
        ...
        const response = await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/facility/${id}`, facilityData);
