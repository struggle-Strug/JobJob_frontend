...
const Message = () => {
    ...
    const getMyMessages = async () => {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/message/user/${user?._id}`);
        setAllMessages(res.data.messages);
        setUnRepliedMessages(res.data.messages.filter(message => message.content[message.content.length-1].sender !== user?._id));
    }
            ...
            <div className="flex flex-col items-center w-full bg-white rounded-t-lg p-4 shadow-xl mt-4">
                <div className="flex justify-center w-3/5">
                    <div className="flex items-center justify-between w-full">
                        <button className={`lg:text-sm md:text-xs text-xs font-bold text-[#343434] hover:text-[#FF2A3B] duration-300 ${showUnreplied ? 'text-[#FF2A3B]' : ''}`} onClick={onClickUnreplied}>未返信</button>
                        <button className={`lg:text-sm md:text-xs text-xs font-bold text-[#343434] hover:text-[#FF2A3B] duration-300 ${showAll ? 'text-[#FF2A3B]' : ''}`} onClick={onClickAll}>すべて</button>
                    </div>
                </div>
            </div>
                            ...
                            {unRepliedMessages?.length > 0 ?
                                <>
                                    {unRepliedMessages.map((message, index) => {
                                        return (
                                            <Link to={`/members/message/${message.message_id}`} key={index} className="flex items-center justify-center w-full border-[1px] border-[#e7e7e7] rounded-lg bg-slate-100 px-2 hover:px-4 py-1 hover:cursor-pointer duration-300">
                                                <img src={message?.facility_id?.photo} alt="" className="h-14 rounded-lg mr-2" />
                                                <div className='flex flex-col'>
                                                    ...
                                                    <p className=" text-xs text-[#858585] line-clamp-2 mt-2">{message?.content[message?.content?.length-1]?.message}</p>
                                                </div>
                                            </Link>
                                        )
                                    })}
                            ...
                            {allMessages?.length > 0 ?
                                <>
                                    {allMessages?.map((message, index) => {
                                        return (
                                            <Link to={`/members/message/${message.message_id}`} key={index} className="flex items-center w-full border-[1px] border-[#e7e7e7] rounded-lg bg-slate-100 px-2 hover:px-4 py-1 hover:cursor-pointer duration-300">
                                                <img src={message?.facility_id?.photo} alt="" className="h-14 rounded-lg mr-2" />
                                                <div className='flex flex-col'>
                                                    <p className="lg:text-2xl md:text-xl text-lg font-bold text-[#343434] hover:text-[#FF2A3B] duration-300 w-full">
                                                        {message?.facility_id?.name}
