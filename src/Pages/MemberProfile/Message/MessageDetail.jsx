...
const MessageDetail = () => {
    ...
    const getMessage = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/message/${message_id}`);
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/file/multi`, formData, {
                headers: {
    ...
    const sendMessage = async () => {
        ...
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/message/send`, messageData)
        ...
        <>
            <div className="flex flex-col w-full h-full">
                <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl">
                    <p className="lg:text-2xl md:text-xl text-lg font-bold text-[#343434]">{message?.facility_id?.name}({message?.jobpost_id?.type})</p>
                    <Link to={`/${jobType}/details/${message?.jobPost_id}`} className='hover:underline hover:text-[#FF2A3B] duration-300 text-[#343434] text-xs mt-1'>求人を確認する</Link>
                </div>
                <div className="flex flex-col w-full bg-white rounded-t-lg p-4 shadow-xl mt-4">
                    <div className='flex flex-col items-end'>
                        <div className='flex flex-col'>
                            <p className='text-sm text-center bg-[#FF2A3B] rounded-t-lg py-1'>応募</p>
                    ...
                    {message?.content?.filter((_, index) => index !== 0)?.map((message, index) => {
                        return (
                            <>
                            <div className={`flex flex-col ${message.sender == user?._id ? "items-end" : "items-start"} mt-4`}>
                                ...
                                {message?.files.length > 0 && 
                                    message?.files?.map((file, index) => {
                                        return (
                                            <a 
                                                key={index}
                                                href={`${process.env.REACT_APP_API_URL}/api/v1/file/download/${file.fileUrl.split("/").pop()}`} 
                     ...
                     <div className="w-full max-w-2xl mx-auto mt-8 textarea">
                        ...
                        >
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                <p className="text-xs">※添付ファイルは送信先の事業所には公開されません</p>
                            </div>
                        </Upload>
                        <TextArea placeholder="返信を書いてください" className="bg-background mt-2" onChange={(e) => setContent(e.target.value)}/>
                        <div className="flex justify-center mt-8">
                            <Button className="bg-rose-500 hover:bg-rose-600 text-white px-8" onClick={sendMessage}>送信する</Button>
                        </div>
                    </div>
                </div>
            </div>
