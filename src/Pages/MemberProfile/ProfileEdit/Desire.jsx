...
const Desire = () => {
  ...
  const employmentDateOptions = [
    ...
    {
      value: "機会があれば転職を検討したい",
      label: "機会があれば転職を検討したい",
    },
  ];
  ...
  const onPeriodChange = (jobType, period) => {
    setJobTypes((prevState) =>
      prevState.map((job) => (job.type === jobType ? { ...job, period } : job))
    );
  };

  const onChangeFeature = (values, category) => {
    const otherCategories = Object.keys(Features).filter(
      (key) => key !== category
    );
  ...
  const renderJobItem = (jobType) => {
    ...
    return (
      <div
        key={jobType}
        className="flex items-center justify-between py-1 mt-1 lg:text-base md:text-sm text-xs bg-red-100 pr-2 rounded-lg"
      >
        <Checkbox
          checked={isSelected}
  ...
  const handleSave = async () => {
    ...
    const res = await axios.post(
      `${process.env.REACT_APP_API_URL}/api/v1/user/${user?._id}/update/desire`,
      payload
    );
  ...
  useEffect(() => {
    ...
    setPrefectures(
      user?.desirePrefecture.length > 0
        ? user?.desirePrefecture
        : Array(user?.prefecture)
    );
    ...
    setToggleMedical(
      user?.jobType?.some((item) =>
        Object.keys(JobTypes.医科).includes(item.type)
      )
    );
    setToggleDentist(
      user?.jobType?.some((item) =>
        Object.keys(JobTypes.歯科).includes(item.type)
      )
    );
    setToggleNursing(
      user?.jobType?.some((item) =>
        Object.keys(JobTypes.介護).includes(item.type)
      )
    );
    setToggleChildcare(
      user?.jobType?.some((item) =>
        Object.keys(JobTypes.保育).includes(item.type)
      )
    );
    setToggleRehabilitation(
      user?.jobType?.some((item) =>
        Object.keys(JobTypes["リハビリ／代替医療"]).includes(item.type)
      )
    );
    setToggleOther(
      user?.jobType?.some((item) =>
        Object.keys(JobTypes.その他).includes(item.type)
      )
    );
    setToggleHealthcare(
      user?.jobType?.some((item) =>
        Object.keys(JobTypes["ヘルスケア／美容"]).includes(item.type)
      )
    );
    setToggleFeature_1(
      user?.feature?.some((item) =>
        Object.keys(Features.HOLIDAY).includes(item)
      )
    );
    setToggleFeature_2(
      user?.feature?.some((item) =>
        Object.keys(Features.WORKING_HOURS).includes(item)
      )
    );
    setToggleFeature_3(
      user?.feature?.some((item) => Object.keys(Features.ACCESS).includes(item))
    );
    setToggleFeature_4(
      user?.feature?.some((item) =>
        Object.keys(Features.DESCRIPTION).includes(item)
      )
    );
    setToggleFeature_5(
      user?.feature?.some((item) =>
        Object.keys(Features.SALARY_BENEFITS_WELFARE).includes(item)
      )
    );
    setToggleFeature_6(
      user?.feature?.some((item) =>
        Object.keys(Features.SERVICE_TYPES).includes(item)
      )
    );
    setToggleFeature_7(
      user?.feature?.some((item) =>
        Object.keys(Features.EDUCATION).includes(item)
      )
    );
    setToggleFeature_8(
      user?.feature?.some((item) =>
        Object.keys(Features.MEDICAL_DEPARTMENT).includes(item)
      )
    );
      ...
      <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl mt-2">
          ...
          <div className="flex flex-col w-3/5">
            <div className="flex flex-col border-t-[0.1rem] border-[#a7a3a3] py-4 px-2 desire">
              <div className="w-full gap-2 ">
                <p
                  className="lg:text-base md:text-sm text-xs text-[#FF2A3B] flex items-center justify-between cursor-pointer duration-300"
                ...
                >
                  ...
                  <img
                    ...
                    className={`duration-300 ${
                      !toggleMedical ? "rotate-90" : "-rotate-90"
                    }`}
                  />
                </p>
              </div>
              <div
                className={`duration-300 overflow-hidden ${
                  toggleMedical ? "opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="mt-4">
            ...
            <div className="flex flex-col border-t-[0.1rem] border-[#a7a3a3] py-4 px-2 desire">
              <div className="w-full gap-2 ">
                ...
                >
                  ...
                  <img
                    ...
                    alt="arrow"
                    className={`duration-300 ${
                      !toggleDentist ? "rotate-90" : "-rotate-90"
                    }`}
                  />
                </p>
              </div>
              <div
                className={`duration-300 overflow-hidden ${
                  toggleDentist ? "opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="mt-4">{dentistKeys.map(renderJobItem)}</div>
              </div>
            </div>
            <div className="flex flex-col border-t-[0.1rem] border-[#a7a3a3] py-4 px-2 desire">
              <div className="w-full gap-2 ">
                ...
                >
                  ...
                  <img
                    ...
                    alt="arrow"
                    className={`duration-300 ${
                      !toggleNursing ? "rotate-90" : "-rotate-90"
                    }`}
                  />
                </p>
              </div>
              <div
                className={`duration-300 overflow-hidden ${
                  toggleNursing ? "opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="mt-4">{nursingKeys.map(renderJobItem)}</div>
              </div>
            </div>
            <div className="flex flex-col border-t-[0.1rem] border-[#a7a3a3] py-4 px-2 desire">
              <div className="w-full gap-2 ">
                ...
                >
                  ...
                  <img
                    ...
                    alt="arrow"
                    className={`duration-300 ${
                      !toggleChildcare ? "rotate-90" : "-rotate-90"
                    }`}
                  />
                </p>
              </div>
              <div
                className={`duration-300 overflow-hidden ${
                  toggleChildcare ? "opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="mt-4">{childcareKeys.map(renderJobItem)}</div>
              </div>
            </div>
            <div className="flex flex-col border-t-[0.1rem] border-[#a7a3a3] py-4 px-2 desire">
              <div className="w-full gap-2 ">
                ...
                >
                  ...
                  <img
                    ...
                    alt="arrow"
                    className={`duration-300 ${
                      !toggleRehabilitation ? "rotate-90" : "-rotate-90"
                    }`}
                  />
                </p>
              </div>
              <div
                className={`duration-300 overflow-hidden ${
                  toggleRehabilitation ? "opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="mt-4">
            ...
            <div className="flex flex-col border-t-[0.1rem] border-[#a7a3a3] py-4 px-2 desire">
              <div className="w-full gap-2 ">
                ...
                >
                  ...
                  <img
                    ...
                    alt="arrow"
                    className={`duration-300 ${
                      !toggleOther ? "rotate-90" : "-rotate-90"
                    }`}
                  />
                </p>
              </div>
              <div
                className={`duration-300 overflow-hidden ${
                  toggleOther ? "opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="mt-4">{otherKeys.map(renderJobItem)}</div>
              </div>
            </div>
            <div className="flex flex-col border-t-[0.1rem] border-[#a7a3a3] py-4 px-2 desire">
              <div className="w-full gap-2 ">
                ...
                >
                  ...
                  <img
                    ...
                    alt="arrow"
                    className={`duration-300 ${
                      !toggleHealthcare ? "rotate-90" : "-rotate-90"
                    }`}
                  />
                </p>
              </div>
              <div
                className={`duration-300 overflow-hidden ${
                  toggleHealthcare ? "opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="mt-4">{healthcareKeys.map(renderJobItem)}</div>
              </div>
            </div>
          </div>
        </div>
              ...
              prefectures?.map((prefecture, index) => {
                return (
                  <Select
                    ...
                    className="w-1/2"
                    value={prefecture}
                    onChange={(value) => handlePrefectureChange(value, index)}
                  />
                );
              })}
        ...
        <div className="flex items-start justify-start w-full mt-4">
          ...
          <div className="flex flex-col items-start justify-start gap-2 w-3/5">
            <Select
              ...
              onChange={(value) => setEmploymentDate(value)}
              className="w-1/2"
            />
          </div>
        </div>
        <div className="flex items-start justify-start w-full mt-4">
          ...
          <div className="flex items-start justify-start gap-2 w-3/5">
            <Input
              value={yearSalary}
              onChange={(value) => setYearSalary(value)}
              className="w-1/3"
            />
            万円
          </div>
            ...
            {[
              {
                ...
                toggle: toggleFeature_1,
                setToggle: setToggleFeature_1,
                category: "HOLIDAY",
              },
              {
                title: "勤務時間",
                options: featureOptions_2,
                toggle: toggleFeature_2,
                setToggle: setToggleFeature_2,
                category: "WORKING_HOURS",
              },
              {
                title: "アクセス",
                options: featureOptions_3,
                toggle: toggleFeature_3,
                setToggle: setToggleFeature_3,
                category: "ACCESS",
              },
              {
                title: "仕事内容",
                options: featureOptions_4,
                toggle: toggleFeature_4,
                setToggle: setToggleFeature_4,
                category: "DESCRIPTION",
              },
              {
                title: "給与・待遇・福利厚生",
                options: featureOptions_5,
                toggle: toggleFeature_5,
                setToggle: setToggleFeature_5,
                category: "SALARY_BENEFITS_WELFARE",
              },
              {
                title: "サービス形態",
                options: featureOptions_6,
                toggle: toggleFeature_6,
                setToggle: setToggleFeature_6,
                category: "SERVICE_TYPES",
              },
              {
                title: "教育体制・教育",
                options: featureOptions_7,
                toggle: toggleFeature_7,
                setToggle: setToggleFeature_7,
                category: "EDUCATION",
              },
              {
                title: "診療科目",
                options: featureOptions_8,
                toggle: toggleFeature_8,
                setToggle: setToggleFeature_8,
                category: "MEDICAL_DEPARTMENT",
              },
            ].map((feature, index) => (
              <div
                key={index}
                className={`flex flex-col border-t-[0.1rem] ${
                  index === 7 ? "border-b-[0.1rem]" : ""
                } border-[#a7a3a3] py-4 px-2 desire`}
              >
                <div className="w-full gap-2">
                  <p
                    className="lg:text-base md:text-sm text-xs text-[#FF2A3B] flex items-center justify-between cursor-pointer duration-300"
                  ...
                  >
                    ...
                    <img
                      ...
                      alt="arrow"
                      className={`duration-300 ${
                        !feature.toggle ? "rotate-90" : "-rotate-90"
                      }`}
                    />
                  </p>
                </div>
                <div
                  className={`duration-300 overflow-hidden ${
                    feature.toggle ? "opacity-100" : "max-h-0 opacity-0"
                  }`}
                >
                  <div className="mt-4">
                    {/* {renderCheckboxGroup(feature.options, onChangeFeature, asks, feature.category)} */}
                    {feature.options.map((option) =>
                      renderFeatureItem(option.value, feature.category)
                    )}
                  </div>
                </div>
              </div>
