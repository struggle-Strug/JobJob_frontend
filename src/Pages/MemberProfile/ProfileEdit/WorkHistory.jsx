...
import { EmploymentType, JobType, Paysystems } from "../../../utils/constants/categories";
...
const WorkHistory = () => {
    ...
    const validateWorkHistory = (history) => {
        const requiredFields = [
            'companyName', 'contents', 'startYear', 'startMonth', 
            'endYear', 'endMonth', 'employmentType', 'jobType', 'jobTypeDetail', 
            'workContent', 'officialPosition', 'payType', 'amount'
        ];
    ...
    const handleGenerate = () => {
        ...
        if (!validateWorkHistory(lastHistory).status) {
            if(validateWorkHistory(lastHistory).field === "companyName") message.error('勤務先名を入力してください。');
            if(validateWorkHistory(lastHistory).field === "contents") message.error('事業内容を入力してください。');
            if(validateWorkHistory(lastHistory).field === "startYear") message.error('勤務開始年を入力してください。');
            if(validateWorkHistory(lastHistory).field === "startMonth") message.error('勤務開始月を入力してください。');
            if(validateWorkHistory(lastHistory).field === "endYear") message.error('勤務終了年を入力してください。');
            if(validateWorkHistory(lastHistory).field === "endMonth") message.error('勤務終了月を入力してください。');
            if(validateWorkHistory(lastHistory).field === "employmentType") message.error('勤務形態を入力してください。');
            if(validateWorkHistory(lastHistory).field === "jobType") message.error('職種を入力してください。');
            if(validateWorkHistory(lastHistory).field === "jobTypeDetail") message.error('職種はどちらも入力してください。');
            if(validateWorkHistory(lastHistory).field === "workContent") message.error('仕事内容を入力してください。');
            if(validateWorkHistory(lastHistory).field === "officialPosition") message.error('役職を入力してください。');
            if(validateWorkHistory(lastHistory).field === "payType") message.error('給与形態を入力してください。');
            if(validateWorkHistory(lastHistory).field === "amount") message.error('給与額を入力してください。');
            return;
        }
    ...
    const handleSave = async () => {
        ...
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/user/${user?._id}/update/work_history`, workDatas);
    ...
    useEffect(() => {
        user?.workHistories.length > 0 && setWorkHistories(user?.workHistories?.map(workHistory => ({
            companyName: workHistory.companyName,
            contents: workHistory.contents,
...
const WorkHistoryEntry = ({ history, index, updateHistory, handleDeleteWorkHistory, facilitiesOptions, employmentTypeOptions, jobTypesOptions, jobTypeDetailOptions, officalPositionOptions, PayTypeOptions, yearsOptions, monthsOptions }) => (
    <div key={index} className="w-full">
        ...
        <div className="flex items-center justify-center w-full mt-4">
            <div className="flex items-center justify-start gap-2 w-2/5">
                <span className="lg:text-base md:text-sm text-xs text-[#343434] font-bold">事業内容</span>
            </div>
            <div className="flex items-center justify-start gap-2 w-full">
                <Select
                    placeholder="事業内容"
        ...
        <div className="flex items-center justify-center w-full mt-4">
            <div className="flex items-center justify-start gap-2 w-2/5">
                <span className="lg:text-base md:text-sm text-xs text-[#343434] font-bold">勤務開始</span>
            </div>
            <div className="flex items-center justify-start gap-2 w-full lg:text-base md:text-sm text-xs">
                <Select placeholder="年" options={yearsOptions} value={history.startYear} onChange={(value) => updateHistory('startYear', value)} className="w-1/4"/>
                年
                <Select placeholder="月" options={monthsOptions} value={history.startMonth} onChange={(value) => updateHistory('startMonth', value)} className="w-1/4"/>
                月
            </div>
        </div>
        <div className="flex items-center justify-center w-full mt-4">
            <div className="flex items-center justify-start gap-2 w-2/5">
                <span className="lg:text-base md:text-sm text-xs text-[#343434] font-bold">勤務終了</span>
            </div>
            <div className="flex items-center justify-start gap-2 w-full lg:text-base md:text-sm text-xs">
                <Select placeholder="年" options={yearsOptions} value={history.endYear} onChange={(value) => updateHistory('endYear', value)} className="w-1/4"/>
                年
                <Select placeholder="月" options={monthsOptions} value={history.endMonth} onChange={(value) => updateHistory('endMonth', value)} className="w-1/4"/>
                月
            </div>
        </div>
        <div className="flex items-center justify-center w-full mt-4">
            <div className="flex items-center justify-start gap-2 w-2/5">
                <span className="lg:text-base md:text-sm text-xs text-[#343434] font-bold">勤務形態</span>
            </div>
            <div className="flex items-center justify-start gap-2 w-full">
                <Select placeholder="勤務形態" options={employmentTypeOptions} value={history.employmentType} onChange={(value) => updateHistory('employmentType', value)} className="w-1/2"/>
            </div>
        </div>
        <div className="flex items-center justify-center w-full mt-4">
            <div className="flex items-center justify-start gap-2 w-2/5">
                <span className="lg:text-base md:text-sm text-xs text-[#343434] font-bold">職種</span>
            </div>
            <div className="flex items-center justify-start gap-2 w-full">
        ...
        <div className="flex justify-start w-full mt-4">
            ...
            <div className="flex flex-col items-start justify-start gap-2 w-full textarea">
                ...
                <TextArea placeholder="仕事内容を入力してください" value={history.workContent} onChange={(e) => updateHistory('workContent', e.target.value)} className="w-4/5"/>
            </div>
        </div>
        <div className="flex items-center justify-center w-full mt-4">
            <div className="flex items-center justify-start gap-2 w-2/5">
                <span className="lg:text-base md:text-sm text-xs text-[#343434] font-bold">役職</span>
            </div>
            <div className="flex items-center justify-start gap-2 w-full">
                <Select placeholder="役職" options={officalPositionOptions} value={history.officialPosition} onChange={(value) => updateHistory('officialPosition', value)} className="w-1/2"/>
            </div>
        </div>
        <div className="flex items-center justify-center w-full mt-4">
            <div className="flex items-center justify-start gap-2 w-2/5">
                <span className="lg:text-base md:text-sm text-xs text-[#343434] font-bold">給与</span>
            </div>
            <div className="flex items-center justify-start gap-2 w-full lg:text-base md:text-sm text-xs">
                <Select placeholder="形態" options={PayTypeOptions} value={history.payType} onChange={(value) => updateHistory('payType', value)} className="w-1/2"/>
                <Input placeholder="" value={history.amount} onChange={(e) => updateHistory('amount', e.target.value)} className="w-1/4"/>
                円
            </div>
        </div>
    </div>
