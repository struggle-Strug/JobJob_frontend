...
const Resumes = () => {
  ...
  const [resumeTitle, setResumeTitle] = useState(
    `${initialDate.split("-")[0]}年${initialDate.split("-")[1]}月${
      initialDate.split("-")[2]
    }日`
  );
  ...
  const getAllRirekis = useCallback(async () => {
    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/v1/rireki/all/${user?._id}`
    );
  ...
  const getAllCareerSheets = useCallback(async () => {
    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/v1/career/all/${user?._id}`
    );
  ...
  const handleRireki = async () => {
    ...
    const res = await axios.post(
      `${process.env.REACT_APP_API_URL}/api/v1/rireki`,
      newRireki
    );
  ...
  return (
    <>
        ...
        <div className="flex flex-col w-full h-full">
          <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl">
            ...
            <div className="flex items-center justify-center gap-4 w-full mt-4">
              <button
                onClick={handleTitleModalOpen}
                className="bg-[#e9e9e9] hover:shadow-xl w-1/2 text-center font-bold lg:text-lg md:text-sm text-xs duration-500 text-[#FF2A3B] hover:text-[#343434] px-2 lg:py-4 md:py-2 py-1 rounded-lg"
              >
                履歴書を作成する
              </button>
              <Link
                to={"/members/resumes/careersheets/new"}
                className="bg-[#e9e9e9] hover:shadow-xl w-1/2 text-center font-bold lg:text-lg md:text-sm text-xs duration-500 text-[#FF2A3B] hover:text-[#343434] px-2 lg:py-4 md:py-2 py-1 rounded-lg"
              >
                職務経歴書を作成する
              </Link>
            ...
            <Link
              to={"/members/profiles"}
              className=" lg:text-[1rem] py-1 md:text-sm text-xs duration-500 text-[#FF2A3B] hover:underline rounded-lg mt-2"
            >
              プロフィールを見る
            </Link>
          </div>
                  ...
                  rirekis?.map((rireki) => {
                    return (
                      <Link
                        ...
                        className="lg:text-xl md:text-lg text-base font-bold text-[#343434] w-full border-[#e7e7e7] border-t-[1px] py-2"
                      >
                        {rireki?.title}
                      </Link>
                    );
          ...
          {careerSheets?.length > 0 && (
            <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl mt-8">
              ...
              <div className="flex flex-col items-start justify-start w-full mt-4 border-[#e7e7e7] border-b-[1px]">
                {careerSheets?.length !== 0 &&
                  careerSheets?.map((careerSheet) => {
                    return (
                      <Link
                        to={`/members/resumes/careersheets/detail/${careerSheet?._id}`}
                        className="lg:text-xl md:text-lg text-base font-bold text-[#343434] w-full border-[#e7e7e7] border-t-[1px] py-2"
                      >
                        {careerSheet?.title}
                      </Link>
        ...
        >
          <div className="flex flex-col w-full h-full modal">
            ...
            <div className="flex items-center justify-start gap-4 w-full mt-4 px-4 py-1">
              ...
              <span className="lg:text-base md:text-sm text-xs text-[#FF2A3B]">
                (必須)
              </span>
              <div className="flex items-start justify-start gap-2 w-2/3">
                <Input
                  placeholder={`履歴書のタイトル`}
                  value={resumeTitle}
      ...
      {pathname.split("/").includes("rireki") &&
        pathname.split("/").includes("detail") && <Rireki />}
      {pathname.split("/").includes("rireki") &&
        pathname.split("/").includes("edit") && <RirekiEdit />}
