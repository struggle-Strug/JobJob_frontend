...
const CareersheetsDetail = ({ careerSheet, path }) => {
  ...
  const createCareerSheet = async () => {
    ...
    const resData = await axios.post(
      `${process.env.REACT_APP_API_URL}/api/v1/career`,
      careerData
    );
  ...
  const updateCareerSheet = async () => {
    ...
    const resData = await axios.put(
      `${process.env.REACT_APP_API_URL}/api/v1/career/update/${careerSheet?._id}`,
      careerData
    );
    if (resData.data.error) return message.error(resData.data.message);
    message.success(resData.data.message);
  ...
  return (
    <div className="flex flex-col w-full h-full">
      <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl">
        ...
        <div className="flex items-center justify-center gap-4 w-full mt-4"></div>
      </div>
      <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl mt-6">
        ...
        <div className="flex items-start justify-center w-full mt-4">
          ...
          <div className="flex flex-col items-start justify-start gap-2 w-3/5">
            <Input
              placeholder="〇〇用の職務経歴書"
              value={title}
              className="lg:text-base md:text-sm text-xs p-2"
              onChange={(e) => setTitle(e.target.value)}
            />
            <p className="lg:text-sm md:text-xs text-xs text-[#343434]">
              ※書類の名前は提出先の事業所には公開されません
            </p>
          </div>
        </div>
        <div className="flex items-start justify-center w-full mt-6">
          ...
          <div className="flex flex-col items-start justify-start gap-2 w-3/5">
            <button
              className="lg:text-base md:text-sm text-xs w-full text-[#FF2A3B] hover:text-white bg-[#ffdbdb] hover:bg-red-500 rounded-lg px-4 py-2 duration-300"
              onClick={openPdf}
            >
              プレビュー
            </button>
            <p className="lg:text-sm md:text-xs text-xs text-[#343434]">
              ※書類に<span className="text-[#FF2A3B]">不備がないか</span>
              確認し、記入漏れの場合は プロフィールを編集しましょう
            </p>
          </div>
        </div>
        {path == "new" ? (
          <div className="flex items-start justify-center w-full mt-6 border-t-[1px] border-[#c4c4c4] pt-8">
            <button
              className="lg:text-base md:text-sm text-xs bg-[#ff6e7a] text-white rounded-lg px-6 py-3 hover:bg-[#ffe4e4] hover:text-red-500 duration-300"
              onClick={createCareerSheet}
            >
              作成する
            </button>
          </div>
