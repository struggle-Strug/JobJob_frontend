...
const DesireEdit = ({ rireki }) => {
  ...
  const handleSave = async () => {
    const resData = await axios.post(
      `${process.env.REACT_APP_API_URL}/api/v1/rireki/update/desire/${rireki?._id}`,
      desire
    );
  ...
  return (
    <>
          ...
          <div className="flex items-start justify-center w-full mt-2">
            ...
            <div className="flex flex-col items-start justify-start gap-2 w-4/5 textarea">
              <TextArea
                ...
                placeholder="例文を挿入できます"
                value={desire.applyReason}
                onChange={(e) =>
                  setDesire({ ...desire, applyReason: e.target.value })
                }
              />
              <button
                className="lg:text-base md:text-sm text-xs bg-[#ff6e7a] text-white rounded-lg px-2 py-1 hover:bg-[#ffe4e4] hover:text-red-500 duration-300"
                onClick={() => setEgApplyReasonOpen(true)}
              >
                志望動機の例文を挿入する
              </button>
            </div>
          ...
          <div className="flex items-start justify-center w-full mt-4">
            ...
            <div className="flex flex-col items-start justify-start gap-2 w-4/5 textarea">
              <TextArea
                ...
                placeholder="例文を挿入できます"
                value={desire.hope}
                onChange={(e) => setDesire({ ...desire, hope: e.target.value })}
              />
              <p className="lg:text-sm md:text-xs text-xs text-[#343434]">
                ※特に待遇・職種・勤務時間・その他について希望があれば入力してください。
              </p>
              <button
                className="lg:text-base md:text-sm text-xs bg-[#ff6e7a] text-white rounded-lg px-2 py-1 hover:bg-[#ffe4e4] hover:text-red-500 duration-300"
                onClick={() => setEgHopeOpen(true)}
              >
                本人希望の例文を挿入する
              </button>
            </div>
      ...
      <ApplyReasonModal
        isOpen={egApplyReasonOpen}
        onClose={() => setEgApplyReasonOpen(false)}
        desire={desire}
        setDesire={setDesire}
      />
      <HopeModal
        isOpen={egHopeOpen}
        onClose={() => setEgHopeOpen(false)}
        desire={desire}
        setDesire={setDesire}
      />
    </>
...
const ApplyReasonModal = ({ isOpen, onClose, desire, setDesire }) => {
  ...
  const templates = [
    ...
    },
    {
      title: "作業療法士",
      content:
        "総合病院で6年間、幼児から高齢者まで幅広い患者様の症例を担当しました。多くの患者様と接する中で、精神障害を抱えた方一人ひとりと向き合い、生活機能を高められる作業療法士になりたいと考えるようになり、精神科病院である貴院を志望しました。また現在は、認定作業療法士の資格取得に向けて準備中です。" +
        "将来的には副チーム長を務めた経験を活かして、組織力の高いチーム作りにも携わりたいと考えています。",
    },
    {
      title: "整体師・セラピスト",
      content:
        "現在はベッド数4台の小さな整体院で働いていますが、治療家としてさらなるステップアップをするため、業界でもトップクラスの技術力と知名度を持つ貴社への転職を志望しました。" +
...
const HopeModal = ({ isOpen, onClose, desire, setDesire }) => {
  ...
  const templates = [
    ...
    },
    {
      title: "入社希望時期",
      content:
        "現職の退職予定が○年○月○日のため、就業は○年○月○日以降を希望します。",
    },
    {
      title: "希望勤務時間",
