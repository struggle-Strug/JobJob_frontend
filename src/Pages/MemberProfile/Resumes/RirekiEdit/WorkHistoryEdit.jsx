...
const WorkHistoryEdit = ({rireki}) => {
    ...
    const handleSave = async () => {
        ...
        if(workHistories.length === 0) {
            const resData = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/rireki/update/work_history/${rireki._id}`, workDatas);
        ...
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/rireki/update/work_history/${rireki._id}`, workDatas);
    ...
    useEffect(() => {
        setWorkHistories(rireki?.workhistory?.length > 0 ? rireki?.workhistory?.map(workHistory => ({
                companyName: workHistory.companyName,
                notes: workHistory.notes,
        ...
        <>
            <div className="flex flex-col w-full h-full">
                <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl">
                    <p className="lg:text-2xl md:text-xl text-lg font-bold text-[#343434]">職歴</p>
                    <p className="lg:text-base md:text-sm text-xs text-[#343434]">履歴書には入職年月の古い順で自動で並び変えられます</p>
            ...
            <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl mt-2">
                ...
                <div className="flex items-start justify-center w-full mt-6">
                    ...
                    <div className="flex items-start justify-start gap-2 w-3/5 desire">
                        {<Checkbox onChange={() => setWorkHistories([{
                                                companyName: "",
                                                notes: [""],
                                                startYear: "",
                                                startMonth: "",
...
const WorkHistoryEditEntry = ({workHistory, workHistories, setWorkHistories, index, updateWorkHistory, yearsOptions, monthsOptions, endStatusOptions, resignationReasonOptions, handleDeleteWorkHistory}) => {
    return (
        <div className="flex flex-col items-start justify-start w-full mt-2">
            <div className="flex items-center justify-between w-full">
                <p className="lg:text-2xl md:text-xl text-lg font-bold text-[#343434]">経歴<span className="number">{index + 1}</span></p>
                <button
                        ...
                        return (
                                    ...
                                    <div key={noteIndex}>
                                            ...
                                            <button 
                                                onClick={() => {
                                                    const newWorkHistories = [...workHistories];
                                                    newWorkHistories[index].notes = workHistory.notes.filter((_, i) => i !== noteIndex);
                                                    setWorkHistories(newWorkHistories);
                                                }}
