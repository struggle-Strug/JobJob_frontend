...
const EducationEdit = ({ rireki }) => {
  ...
  const validateEducation = (history) => {
    const requiredFields = ["schoolName_department_major"];
  ...
  const handleGenerate = () => {
    ...
    if (!validateEducation(lastEducation).status) {
      if (
        validateEducation(lastEducation).field === "schoolName_department_major"
      )
        message.error("学校・学部・学科・専攻名を入力してください。");
      return;
    }
    setEducations([
      ...educations,
  ...
  const handleSave = async () => {
    ...
    if (educations.length === 0) {
      const resData = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/v1/rireki/update/education/${rireki._id}`,
        educationDatas
      );
      ...
      rireki?.education.length > 0
        ? rireki.education.map((education) => {
            return {
              ...
              notes: education.notes,
              admissionYear: education.admissionDate
                ? education.admissionDate.split("-")[0]
                : "",
              admissionMonth: education.admissionDate
                ? education.admissionDate.split("-")[1]
                : "",
              admission: education.admission,
              graduationYear: education.graduationDate
                ? education.graduationDate.split("-")[0]
                : "",
              graduationMonth: education.graduationDate
                ? education.graduationDate.split("-")[1]
                : "",
              graduation: education.graduation,
            };
          })
  ...
  return (
    <div className="flex flex-col items-start justify-start w-full mt-2">
      <div className="flex items-center justify-between w-full">
        <p className="lg:text-2xl md:text-xl text-lg font-bold text-[#343434]">
          学歴<span className="number">{index + 1}</span>
        </p>
        <button
      ...
      <div className="flex items-start justify-center w-full mt-2">
        ...
        <div className="flex flex-col items-start justify-start gap-2 w-3/5">
          <Input
            placeholder="学校・学部・学科・専攻名"
            value={education.schoolName_department_major}
            onChange={(e) => {
              updateEducation("schoolName_department_major", e.target.value);
            }}
            className="p-2"
          />
          <p className="lg:text-sm md:text-xs text-xs text-[#343434]">
            ※名称は省略せず正式名称で表記してください
          </p>
        </div>
      </div>
            ...
            return (
              <>
                ...
                <button
                  onClick={() => {
                    const newEducations = [...educations];
                    newEducations[index].notes = education.notes.filter(
                      (_, i) => i !== noteIndex
                    );
                    setEducations(newEducations);
                  }}
      ...
      <div className="flex items-start justify-center w-full mt-6">
        ...
        <div className="flex items-start justify-start gap-2 w-3/5">
          <Select
            ...
            value={education.admissionYear}
            onChange={(value) => {
              updateEducation("admissionYear", value);
            }}
            className="w-1/3"
          />
          <Select
            options={monthsOptions}
            value={education.admissionMonth}
            onChange={(value) => {
              updateEducation("admissionMonth", value);
            }}
            className="w-1/3"
          />
          <Select
            options={admissionOptions}
            value={education.admission}
            onChange={(value) => {
              updateEducation("admission", value);
            }}
            className="w-1/3"
          />
        </div>
      ...
      <div className="flex items-start justify-center w-full mt-6">
        ...
        <div className="flex items-start justify-start gap-2 w-3/5">
          <Select
            ...
            onChange={(value) => {
              updateEducation("graduationYear", value);
            }}
            className="w-1/3"
          />
          <Select
            ...
            onChange={(value) => {
              updateEducation("graduationMonth", value);
            }}
            className="w-1/3"
          />
          <Select
            ...
            onChange={(value) => {
              updateEducation("graduation", value);
            }}
            className="w-1/3"
          />
        </div>
