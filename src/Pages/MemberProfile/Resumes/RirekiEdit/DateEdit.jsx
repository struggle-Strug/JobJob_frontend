...
const DateEdit = ({rireki}) => {
    ...
    const handleSave = async () => {
        ...
        const resData = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/rireki/update/date/${rireki?._id}`, {creationDate})
    ...
    return (
        <div className="flex flex-col w-full h-full">
            <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl">
                <p className="lg:text-2xl md:text-xl text-lg font-bold text-[#343434]">履歴書の作成日</p>
            ...
            <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl mt-2">
                <div className="flex items-start justify-center w-full mt-2">
                    <div className="flex items-center justify-start gap-2 w-1/3">
                        <span className="lg:text-base md:text-sm text-xs font-bold text-[#343434]">履歴書の作成日</span>
                    </div>
                    <div className="flex items-start justify-start gap-2 w-2/3 textarea">
                            <Select options={yearsOptions} className="w-1/3" value={year} onChange={(value) => setYear(value)}/>
                            <Select options={monthsOptions} className="w-1/3" value={month} onChange={(value) => setMonth(value)}/>
                            <Select options={daysOptions} className="w-1/3" value={day} onChange={(value) => setDay(value)}/>
                    </div>
                </div>
