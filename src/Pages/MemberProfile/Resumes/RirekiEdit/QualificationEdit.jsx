...
const QualificationEdit = ({ rireki }) => {
  ...
  const handleQualificationChange = (qualification, checked) => {
    if (!checked) {
      setQualification((prev) =>
        prev.map((q) =>
          q.qualification === qualification ? { ...q, deleted: true } : q
        )
      );
    } else {
      setQualification((prev) =>
        prev.map((q) =>
          q.qualification === qualification ? { ...q, deleted: false } : q
        )
      );
    }
  };
  ...
  const updateQualificationDate = (qualification, field, value) => {
    setQualification((prev) =>
      prev.map((q) =>
        q.qualification === qualification ? { ...q, [field]: value } : q
      )
    );
  };
  ...
  const handleSave = async () => {
    ...
    if (!isQualificationOpen) {
      ...
      const resData = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/v1/rireki/update/qualification/${rireki._id}`,
        qualificationData
      );
    ...
    const realQualifications = qualification.filter(
      (qualification) => !qualification.deleted
    );
    qualificationData = realQualifications.map((qualification) => ({
            ...
            <div className="flex items-center justify-between gap-2 w-full flex-wrap p-4 overflow-scroll h-[40rem] overflow-x-hidden">
              {qualificationKeys.map((qualificationKey) => (
                <Checkbox
                  ...
                  disabled={qualification.some(
                    (detail) => detail.qualification === qualificationKey
                  )}
                  onChange={(e) => {
                    const { checked } = e.target; // Get the checked state
                    if (checked) {
                      // If checked, add the qualification to the state
                      setQualificationOther((prev) => [
                        ...prev,
                        qualificationKey,
                      ]);
                    }
