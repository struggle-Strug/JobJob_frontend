...
const OtherEdit = ({rireki}) => {
    ...
    const handleSave = async () => {
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/rireki/update/other/${rireki?._id}`, other);
    ...
    return (
        <div className="flex flex-col w-full h-full">
            <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl">
                <p className="lg:text-2xl md:text-xl text-lg font-bold text-[#343434]">その他</p>
            ...
            <div className="flex flex-col items-start justify-center w-full bg-white rounded-lg p-4 shadow-xl mt-2">
                <div className="flex items-start justify-center w-full mt-2">
                    <div className="flex items-center justify-start gap-2 w-2/5">
                        <span className="lg:text-base md:text-sm text-xs font-bold text-[#343434]">通勤時間</span>
                    </div>
                    <div className="flex flex-col items-start justify-start gap-2 w-3/5">
                        <Input placeholder="徒歩15分＋電車20分" value={other.time} onChange={(e) => setOther({...other, time: e.target.value})} className="w-2/3"/>
                        <p className="lg:text-sm md:text-xs text-xs text-[#343434]">※片道最短ルートの所要時間を5分単位で書きましょう</p>
                    </div>
                </div>
                <div className="flex items-start justify-center w-full mt-4">
                    <div className="flex items-center justify-start gap-2 w-2/5">
                        <span className="lg:text-base md:text-sm text-xs font-bold text-[#343434]">扶養家族数(配偶者を除く)</span>
                    </div>
                    <div className="flex flex-col items-start justify-start gap-2 w-3/5">
                        <div className="flex items-center justify-start gap-2 w-full">
                            <Input placeholder="扶養家族数(配偶者を除く)" value={other.dependents} onChange={(e) => setOther({...other, dependents: e.target.value})} className="w-1/3"/>
                            <span className="lg:text-base md:text-sm text-xs text-[#343434]">人</span>
                        </div>
                        <p className="lg:text-sm md:text-xs text-xs text-[#343434]">※扶養義務のある家族がいなければ、0と入力してください</p>
                    </div>
                </div>
                <div className="flex items-center justify-center w-full mt-4">
                    <div className="flex items-center justify-start gap-2 w-2/5">
                        <span className="lg:text-base md:text-sm text-xs font-bold text-[#343434]">配偶者</span>
                    </div>
                    <div className="flex items-center justify-start gap-2 w-3/5">
                        <Select options={spouseOptions} value={other.spouse} onChange={(value) => setOther({...other, spouse: value})}  className="w-1/3"/>
                    </div>
                </div>
