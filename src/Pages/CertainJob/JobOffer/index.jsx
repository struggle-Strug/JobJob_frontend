...
const JobOffer = () => {
    ...
    const getJobPost = async () => {
        try {
            ...
            const resData = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/customers/${res.data.jobpost.customer_id.customer_id}`);
    ...
    const handleApply = async () => {
        ...
        if((hiraganaSei || hiraganaMei) === "") return message.error("ふりがな名を入力してください。")
        if((year || month || day) === "") return message.error("生年月日を入力してください。")
        ...
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/v1/message`, messageData)
                            ...
                            <div className="flex flex-col justify-center bg-white rounded-lg px-6 py-6 w-full shadow-xl mt-4">
                                ...
                                <div className="flex justify-between w-full mt-6">
                                    ...
                                    <div className="flex items-start justify-start w-4/5">
                                        {getJobTypeKeyByValue(jobType)}({jobPost?.employment_type[0]})
                                    </div>
                                </div>
                                <div className="flex justify-between w-full mt-6">
                                    <div className="flex items-start gap-2 justify-end">
                                                        ...
                                                        <>
                                                            <div className="flex flex-col w-full">
                                                                <div key={dateIndex} className="flex flex-col bg-[#EFEFEF] rounded-lg p-2 mt-4 w-2/5">
                                                                    <Select options={dateOptions} placeholder="日程を選択" value={meeting.date} onChange={(value) => handleDateChange(value, dateIndex)} className="w-full"/>
                                                                    {meeting.times.map((time, timeIndex) => {
                                                                        return (
                                                                            <>
                                                                                <div key={timeIndex} className="flex mt-4 gap-2 items-center">
                                                                                    <Select options={timeOptions} placeholder="時間を選択" value={time.time} onChange={(value) => handleTimeChange(value, dateIndex, timeIndex)} className="w-1/3"/>
                                                                                    <Select options={minuteOptions} placeholder="分を選択" value={time.minute} onChange={(value) => handleMinuteChange(value, dateIndex, timeIndex)} className="w-1/3"/>
                                                                                    ~
                                ...
                                <div className="w-full border-t-[1px] border-[#EFEFEF] mt-4 text-center p-4">
                                    <button className="lg:text-base md:text-sm text-xs font-bold text-[#FF2A3B] hover:text-white bg-[#ffdbdb] hover:bg-red-500 rounded-lg px-24 py-3 duration-300" onClick={handleApply}>応募する</button>
            ...
            </div>
            {
                ...
                >
                    <div className="flex flex-col items-start justify-start">
                        <h1 className="lg:text-base text-sm font-bold text-[#343434]">{jobPost?.facility_id.name}への応募が完了しました。</h1>
                        <p className="text-xs text-[#343434] mt-4">ご応募ありがとうございます。応募後の流れなどを紹介している応募完了メールを送信しました。質問や追加アピール、履歴書の送付などはメッセージにて応募先へ直接連絡できます。</p>
                        <Link to={"/members/message"} className="mt-4 bg-[#EFEFEF] px-4 py-2 rounded-lg">メッセージに移動する</Link>
                    </div>
                </Modal>
            }
        </>
