...
const JobLists = () => {
  ...
  const renderPrefectureSection = (region, prefectures) => (
    <div className="col-span-1 flex flex-col justify-start items-center">
      ...
      <div className="flex flex-col w-full px-2 lg:px-4">
        {Object.keys(prefectures).map((prefectureName, index) => {
          const prefValue = prefectures[prefectureName];
          // URL に現在のフィルター（市区町村リセット）を反映
          const newFilters = { ...filters, pref: prefValue, muni: "" };
          const href = `/${path}/search?filters=${encodeURIComponent(
    ...
    <>
      <div className="flex flex-wrap w-full px-2 lg:px-4">
        ...
        <div className=" border-t-[1px] border-[#bdbdbd] mt-4">
          {Municipalities[prefecture]?.map((muniName, index) => {
            const newFilters = { ...filters, muni: muniName };
            const href = `/${path}/search?filters=${encodeURIComponent(
              JSON.stringify(newFilters)
            )}`;
            return (
  ...
  const handleOnChangePref = (p) => {
    ...
    if (pathname.split("/")[2] === "search") {
      setFilters((prevFilters) => {
        const newFilters = { ...prevFilters, pref: p, muni: "" };
        navigate(
      ...
      setFilters((prevFilters) => {
        const newFilters = { ...prevFilters, pref: p };
        navigate(`/${path}/${p}`);
        setPrefectureModalOpen(false);
        return newFilters;
      });
  ...
  const handleOnChangeMuni = (m) => {
    setMuni(m);
    setFilters((prevFilters) => {
      const newFilters = { ...prevFilters, muni: m };
      navigate(
        `/${path}/search?filters=${encodeURIComponent(
          JSON.stringify(newFilters)
        )}`
  ...
  const handleOnChangePage = (p) => {
    setPage(p);
    setFilters((prevFilters) => {
      const newFilters = { ...prevFilters, page: p };
      navigate(
        `/${path}/search?filters=${encodeURIComponent(
          JSON.stringify(newFilters)
        )}`
  ...
  const handleEmploymentTypeChange = (employmentTypeValue) => {
    setEmploymentType((prev) =>
      prev.includes(employmentTypeValue)
        ? prev.filter((type) => type !== employmentTypeValue)
        : [...prev, employmentTypeValue]
    );
  };

  const handleFeatureChange = (featureValue) => {
    const key = getFeatureKeyByValue(featureValue);
    setFeature((prev) =>
            ...
            <div className="flex flex-col justify-center bg-white rounded-lg px-12 py-8 w-full shadow-xl mt-8">
              ...
              <div className="flex items-center justify-between py-4 px-8 bg-[#F6F6F6] rounded-lg mt-4 hover:px-12 duration-300 cursor-pointer">
                <div className="flex items-center justify-between gap-1 ">
                  ...
                  <Link
                    to={`/${path}/train-line`}
                    className="lg:text-md md:text-sm font-bold text-[#343434] hover:text-[#FF2A3B]"
                  >
                    沿線から選択
                  </Link>
                </div>
          ...
          <div className="w-full p-6">
            ...
            <div className="flex flex-wrap gap-4 mt-4">
              {Object.keys(EmploymentType).map((employmentTypeKey, index) => {
                // 各項目ごとにチェックとリンクを分離（リンクは当該フィルター適用URL）
                const newFilters = {
                  ...filters,
                ...
                return (
                  <div key={index} className="flex items-center">
                    ...
                    <Link
                      to={href}
                      className="ml-2 text-xs lg:text-sm text-[#343434] hover:text-[#FF2A3B]"
                    >
                      {employmentTypeKey}
                    </Link>
                  </div>
            ...
            <div key={index} className="mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {Object.keys(section.features).map((featureKey, idx) => {
                  const newFilters = {
                    ...filters,
                    feature: [getFeatureKeyByValue(section.features[featureKey])],
                  };
                  const href = `/${path}/search?filters=${encodeURIComponent(
                    JSON.stringify(newFilters)
                  )}`;
